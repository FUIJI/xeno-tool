        ...
        if not date_folders:
            ...
            with ThreadPoolExecutor(max_workers=100) as executor:
                ...
                for i, future in enumerate(as_completed(future_to_date_folder)):
                    ...
                    try:
                        future.result()
                        log_message(f"✅ Organized folder: {date_folder_name} Successfully")
        ...
        if file_path:
            try:
                with open(file_path, "w", encoding="utf-8") as file:  # Specify UTF-8 encoding
                    ...
                    # Append iri_dataframes if it exists
                    if hasattr(self, 'iri_dataframes') and not self.iri_dataframes.empty:
                        file.write("\n\nIRI DataFrames:\n")
                        self.iri_dataframes.to_csv(file, index=False, line_terminator="\n")
                    else:
