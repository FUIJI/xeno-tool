...
#                         re.search(r'xw_iri_qgis_Session', file_name2) or
#                         re.search(r'drp_rutting_qgis_Session', file_name2)):
    ...
    try:
        for root, dirs, files in os.walk(path):
            ...
            for filename in iri_files:
                ...
                if iri_df.empty or len(iri_df.columns) < 6:
                    print(f"⚠️ Skipping {filename}: Invalid data format")
                    continue
                # print(iri_df)
        ...
        print(f"✅ Finished Prepar : .CSV files.")
    except Exception as e:
        print(f"❌ Failed to Prepar : {e}.") 
        
    return iri_dataframes, rutting_dataframes

# 
path = r'E:/RS25/Test/xenomatix'
iri_dataframe, rutting_dataframe = process_csv_files(path)
# 
